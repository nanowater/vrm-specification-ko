...
타입 정의 중 Color와 Texture는 glTF에 준하여 선형 색 공간(Linear Colorspace)에 저장됩니다.

### 정점 색상 (Vertex Colors)

MToon 마테리얼에서는 정점 색상(Vertex Colors)을 무시합니다.
...
|             | 타입     | 설명                | 필수  |
| ----------- | -------- | ------------------- | :---- |
| specVersion | `string` | 이 확장의 버전 번호 | ✅ 예 |
...
툰 셰이더에서는 반투명 표현이 다용되기 때문에 렌더링 순서로 인한 문제가 자주 발생합니다.
MToon에서는 이를 위해 `transparentWithZWrite`를 도입했으나, 추가로 렌더링 순서 제어 구조를 도입하고 있습니다.
`renderQueueOffsetNumber`는 각 Render Mode의 기본 렌더링 순서에 대한 오프셋 값입니다.
...
|                           | 타입      | 설명                                                     | 필수                    |
| :------------------------ | :-------- | :------------------------------------------------------- | :---------------------- |
| `transparentWithZWrite`   | `boolean` | `alphaMode`가 `BLEND`일 때, ZBuffer 쓰기를 수행할지 여부 | 아니오, 초기값: `false` |
| `renderQueueOffsetNumber` | `integer` | 렌더링 순서에 대한 오프셋 값                             | 아니오, 초기값: `0`     |
...
#### 라이팅 모델 (Lighting Model)

MToon은 램버트(Lambert) 반사 모델을 확장합니다.
일반적인 램버트 반사 모델과 마찬가지로 면의 법선과 라이트 벡터에 의해 음영이 생기지만, 다음과 같은 특징을 통해 툰 표현을 가능하게 합니다:

- 일반 베이스 컬러와 별도로 그림자 색(Shade Color)을 지정할 수 있습니다.
- 그림자 경계의 위치 및 넓이(흐림 정도)를 조정할 수 있습니다.

#### Light를 받는 색상 (Lit Color)
...
일반적인 램버트 반사 모델의 경우 빛을 받지 않는 그림자 면은 검게 렌더링되지만, MToon에서는 베이스 컬러와 그림자 색이 선형 보간되는 방식으로 동작합니다.
또한 베이스 컬러가 텍스처에 의해 곱해지는 것과 마찬가지로 그림자 색에 대해서도 전용 텍스처를 별도로 지정할 수 있으며, 텍스처 색상이 그림자 색에 곱해집니다.

그림자 색은 MToon 확장으로 정의되는 `shadeColorFactor` 및 `shadeMultiplyTexture`를 사용합니다.

<img src="figures/mtoon-lit-shade-ko.png" width="67%">

#### 표면 법선 (Surface Normal)

법선 맵(Normal Map)을 이용할 수 있습니다. glTF 코어 사양의 매테리얼 정의에 포함된 `normalTexture`를 사용합니다.

#### 쉐이딩 시프트 (Shading Shift)
...
<img src="figures/mtoon-shading-ramp-ko.png" width="67%">
...
|                      | 타입        | 설명                                     | 필수                         |
| :------------------- | :---------- | :--------------------------------------- | :--------------------------- |
| shadeColorFactor     | `number[3]` | Shade 색                                 | 아니오, Default: `[0, 0, 0]` |
| shadeMultiplyTexture | `object`    | Shade 색의 곱셈 텍스처                   | 아니오                       |
| shadingShiftFactor   | `number`    | 셰이딩 경계를 시프트(shift)하는 값       | 아니오, Default: `0.0`       |
| shadingShiftTexture  | `object`    | 셰이딩 경계를 시프트하는 텍스처          | 아니오                       |
| shadingToonyFactor   | `number`    | 셰이딩 경계의 평활도(블러)를 지정하는 값 | 아니오, Default: `0.9`       |

#### shadeColorFactor

그림자 색을 지정합니다.
값은 선형 색 공간(Linear colorspace)에서 평가됩니다.
...
할당된 텍스처의 RGB 컴포넌트를 참조하여 선형 색 공간으로 변환 후 평가됩니다.
...
구체적인 계산 방법에 대해서는 상단의 [Shading Shift](#shading-shift)를 참조하십시오.

텍스처 값은 선형 색 공간입니다.
할당된 텍스처의 R 컴포넌트를 참조하여 선형 색 공간에서 평가됩니다.
...
`shadingShiftTexture`에서 텍스처를 지정하는 객체입니다.

#### Properties

|          | 타입      | 설명                                      | 필수                  |
| :------- | :-------- | :---------------------------------------- | :-------------------- |
| index    | `integer` | 텍스처의 index                            | ✅ 예                 |
| texCoord | `integer` | 텍스처 맵핑에 사용하는 TEXCOORD           | 아니오, 초기값: `0`   |
| scale    | `number`  | 텍스처의 셰이딩 경계 기여도를 지정하는 값 | 아니오, 초기값: `1.0` |
...
일반적으로 툰 셰이더에서는 음영을 보고 지오메트리의 요철을 상세히 읽을 수 있는 표현은 바람직하지 않습니다.
따라서 본 확장 사양에서는 Shading Toony와 Shading Shift 파라미터를 도입하고 있습니다.
이 파라미터들은 지오메트리의 요철, 즉 법선 방향과 광원의 입사 방향 변화에 대해 음영이 크게 변하지 않도록 제어합니다.

그러나 IBL이나 SH Lighting과 같은 대역 조명에서는 이 파라미터만으로 불충분합니다.
전역 조명은 방향이 변하면 방사 휘도(radiance)도 크게 변하기 때문입니다.

이에 본 확장 사양에서는 GI Equalization Factor 파라미터를 도입합니다.
GI Equalization Factor는 지오메트리 법선에 상관없이 지오메트리가 받는 대역 조명을 일정하게 만들어 줄 수 있습니다.
그 결과 렌더링되는 음영이 약해져 지오메트리의 요철을 읽기 어렵게 만들 수 있습니다.
구체적으로는 대역 조명을 방향에 대해 평활화(균일화)함으로써 이를 실현합니다.
...
하지만 실시간 렌더링에서는 부하가 높기 때문에 각 구현 환경에 맞춰 근사 구현을 사용하십시오.
예를 들어 Spherical Harmonics의 저차 항만 다루는 방식 등을 들 수 있습니다.
단, Spherical Harmonics의 저차 항만 사용하는 경우 강한 평행 광원(예: 태양광) 하에서 근사가 제대로 안 될 수 있습니다.
따라서 여기서는 2점 샘플링의 평균을 근사 구현 예시로 듭니다.
...
|                      | 타입     | 설명                    | 필수                   |
| :------------------- | :------- | :---------------------- | :--------------------- |
| giEqualizationFactor | `number` | 전역 조명의 균일화 계수 | 아니오, Default: `0.9` |
...
구체적인 계산 방법에 대해서는 상단의 [Global Illumination](#global-illumination)을 참조하십시오.
...
MatCap은 View Normal Vector를 기반으로 텍스처를 매핑하는 기법입니다.
주로 사전에 라이팅을 텍스처로 베이킹해 두는 용도로 널리 쓰입니다.
MatCap은 라이팅 결과에 가산(Additive Blend)됩니다.

텍스처는 MToon 확장으로 정의되는 `matcapTexture`에 저장됩니다.
또한 `matcapFactor`에 의해 값이 곱해집니다.

#### 파라메트릭 림 라이팅 (Parametric Rim Lighting)

파라메트릭 림 라이트는 View Normal Vector를 기반으로 오브젝트 가장자리에 가상 림 라이트 효과를 부여하는 기법입니다.
파라메트릭 림 라이트는 라이팅 결과에 가산됩니다.

MToon 확장으로 정의된 `parametricRimColorFactor`라는 값으로 파라메트릭 림 라이트의 색을 제어할 수 있습니다.

또한, MToon 확장으로 정의된 `parametricRimFresnelPowerFactor`, `parametricRimLiftFactor` 값으로 파라메트릭 림 라이트의 모양을 제어할 수 있습니다.
...
#### 림 승산 텍스처 (Rim Multiply Texture)

텍스처를 이용하여 특정 부위에만 림 라이팅을 나타내거나 지우는 제어를 할 수 있습니다.
UV 맵핑된 텍스처 값이 림 라이트 색상에 곱해집니다.

텍스처는 MToon 확장으로 정의되는 `rimMultiplyTexture`에 저장됩니다.

#### 림 라이팅 믹스 (Rim Lighting Mix)

림 라이팅이 주변 광원으로부터 어느 정도의 영향을 받을지를 제어할 수 있습니다.
전혀 영향을 받지 않을 경우, 에미션처럼 스스로 발광하는 림 라이팅이 됩니다.
...
|                                 | 타입        | 설명                                      | 필수                        |
| :------------------------------ | :---------- | :---------------------------------------- | :-------------------------- |
| matcapFactor                    | `number[3]` | MatCap 텍스처에 곱해지는 색               | 아니오, 초기값: `[1, 1, 1]` |
| matcapTexture                   | `object`    | MatCap 텍스처                             | 아니오                      |
| parametricRimColorFactor        | `number[3]` | 파라메트릭 림 라이트의 색                 | 아니오, 초기값: `[0, 0, 0]` |
| parametricRimFresnelPowerFactor | `number`    | 파라메트릭 림 라이트의 프레넬 계수        | 아니오, 초기값: `5.0`       |
| parametricRimLiftFactor         | `number`    | 파라메트릭 림 라이트의 덧셈 항            | 아니오, 초기값: `0.0`       |
| rimMultiplyTexture              | `object`    | 림 라이팅에 대해 곱해지는 텍스처          | 아니오                      |
| rimLightingMixFactor            | `number`    | 림 라이팅이 광원으로부터 받는 영향의 비율 | 아니오, 초기값: `1.0`       |
...
구체적인 계산 방법에 대해서는 상단의 [Parametric Rim Lighting](#parametric-rim-lighting)을 참조하십시오.
...
텍스처의 값은 sRGB 전달 함수로 인코딩되어 있습니다.
할당된 텍스처의 RGB 컴포넌트를 참조하여 선형 색 공간으로 변환해 평가됩니다.
정의되지 않은 경우 RGB의 각 값은 `1.0`으로 평가되어야 합니다.
...
#### 윤곽선 라이팅 믹스 (Outline Lighting Mix)
...
|                             | 타입        | 설명                                         | 필수                        |
| --------------------------- | ----------- | -------------------------------------------- | --------------------------- |
| outlineWidthMode            | `string`    | 윤곽선 렌더링 모드                           | 아니오, 초기값: `"none"`    |
| outlineWidthFactor          | `number`    | 윤곽선 두께                                  | 아니오, 초기값: `0.0`       |
| outlineWidthMultiplyTexture | `object`    | 윤곽선 두께 지정 텍스처                      | 아니오                      |
| outlineColorFactor          | `number[3]` | 윤곽선 색                                    | 아니오, 초기값: `[0, 0, 0]` |
| outlineLightingMixFactor    | `float`     | 윤곽선 색에 표면의 셰이딩 결과를 곱하는 비율 | 아니오, 초기값: `1.0`       |

#### outlineWidthMode

윤곽선의 두께를 어떻게 결정할지를 지정합니다.

구체적인 계산 방법에 대해서는 상단의 [Outline Width](#outline-width)를 참조하십시오.
...
텍스처의 값은 선형 색 공간입니다.
할당된 텍스처의 G 컴포넌트를 참조하여 선형 색 공간에서 평가됩니다.
정의되지 않은 경우 G 컴포넌트의 값은 `1.0`으로 평가되어야 합니다.

> 할당된 텍스처의 G 컴포넌트를 참조하므로, 흑백 마스크 텍스처를 사용할 수도 있고 채널별로 다른 마스크를 가진 RGB 텍스처를 이용할 수도 있습니다.
...
애니메이션은 직접 제어할 수 있는 것이 아니며, 자동적이고 항상 계속되는 방식입니다.
...
> 구현 노트: 본 확장 및 코어 사양 외부에서 정의된 텍스처에 대해서는 별도로 규정하지 않습니다. 필요에 따라 각 구현체별로 대응하십시오.
...
단위는 라디안 (초당)이며, `1.0`일 경우 2π초마다 UV가 1회전 합니다.
...
#### 변환 순서 (Transform Order)

UV 애니메이션 적용 순서를 동차 좌표 벡터(homogeneous coordinate vector)를 이용해 행렬로 표시합니다.

`uvAnimationScrollXSpeedFactor`에 의한 X 방향의 스크롤을 $\mathrm{scrollX}$,
`uvAnimationScrollYSpeedFactor`에 의한 Y 방향의 스크롤을 $\mathrm{scrollY}$,
`uvAnimationRotationSpeedFactor`에 의한 회전 애니메이션을 $\mathrm{rotation}$이라고 할 때,

$$
\begin{pmatrix} \mathrm{uv}'.x \\ \mathrm{uv}'.y \\ 1 \end{pmatrix}
= \begin{bmatrix} 1 & 0 & \mathrm{scrollX} \\ 0 & 1 & \mathrm{scrollY} \\ 0 & 0 & 1 \end{bmatrix}
\begin{bmatrix} 1 & 0 & 0.5 \\ 0 & 1 & 0.5 \\ 0 & 0 & 1 \end{bmatrix}
\begin{bmatrix} \cos(\mathrm{rotation}) & \sin(\mathrm{rotation}) & 0 \\ -\sin(\mathrm{rotation}) & \cos(\mathrm{rotation}) & 0 \\ 0 & 0 & 1 \end{bmatrix}
\begin{bmatrix} 1 & 0 & -0.5 \\ 0 & 1 & -0.5 \\ 0 & 0 & 1 \end{bmatrix}
\begin{pmatrix} \mathrm{uv}.x \\ \mathrm{uv}.y \\ 1 \end{pmatrix}
$$
...
> 평행 이동과 회전이 모두 설정된 텍스처에서 시간 경과에 따라 애니메이션 속도가 가속되지 않는 동작이 올바른 동작입니다.
...
#### KHR_texture_transform과의 호환성

`KHR_texture_transform` 확장을 사용하고 있는 경우, UV 애니메이션에 의한 텍스처의 좌표 변환은 `KHR_texture_transform`에 의한 좌표 변환보다 먼저 실행됩니다.

> `KHR_texture_transform`을 병용하는 경우에도 UV 애니메이션 변환 시 UV 각 요소 범위를 Clamp나 Repeat 등으로 [0.0 - 1.0]으로 제한하지 않습니다.

#### MToon Defined Properties

|                                | 타입     | 설명                                          | 필수                  |
| ------------------------------ | -------- | --------------------------------------------- | --------------------- |
| uvAnimationMaskTexture         | `object` | UV 애니메이션을 수행할 범위를 지정하는 텍스처 | 아니오                |
| uvAnimationScrollXSpeedFactor  | `number` | UV 애니메이션의 X 방향 이동 속도              | 아니오, 초기값: `0.0` |
| uvAnimationScrollYSpeedFactor  | `number` | UV 애니메이션의 Y 방향 이동 속도              | 아니오, 초기값: `0.0` |
| uvAnimationRotationSpeedFactor | `number` | UV 애니메이션의 회전 속도                     | 아니오, 초기값: `0.0` |
